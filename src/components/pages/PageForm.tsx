...
const PageForm = ({ page, closeModal }: { page?: Page; closeModal?: () => void }) => {
  ...
  const { mutate: createPage, isLoading: isCreating } = trpc.pages.createPage.useMutation({
    onSuccess: (res) => onSuccess("create"),
  });

  const { mutate: updatePage, isLoading: isUpdating } = trpc.pages.updatePage.useMutation({
    onSuccess: (res) => onSuccess("update"),
  });

  const { mutate: deletePage, isLoading: isDeleting } = trpc.pages.deletePage.useMutation({
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(handleSubmit)} className={"space-y-8"}>
        ...
        {editing ? (
          <Button type="button" variant={"destructive"} onClick={() => deletePage({ id: page.id })}>
